import dash
from dash import html, dcc
import requests

# Initialiser l'application Dash
app = dash.Dash(__name__, requests_pathname_prefix='/dashboard/')

def get_weather_info():
    try:
        response = requests.get("https://TON-APP-METEO.azurewebsites.net/info")
        return response.json()
    except:
        return {
            "weather": {
                "city": "N/A",
                "temperature": "N/A",
                "description": "Erreur"
            }
        }

# Appeler la météo une seule fois au chargement
weather = get_weather_info()

# Layout avec météo + graphiques
app.layout = html.Div(children=[
    html.Div([
        html.A('Accueil', href='/'),
        "|",
        html.A("Logout", href="/logout")
    ], style={'marginTop': 25}),
    
    html.H1("Example de Dashboard"),

    html.H3(f"Météo à {weather['weather']['city']}"),
    html.P(f"Température : {weather['weather']['temperature']} °C"),
    html.P(f"Description : {weather['weather']['description']}"),
    
    html.H2("*** Bar Graph *** "),
    dcc.Graph(
        id="exm1",
        figure={
            "data": [
                {"x": [5, 7, 12], "y": [10, 16, 11], "type": "bar", "name": "exmple1"},
                {"x": [8, 18, 22], "y": [5, 8, 3], "type": "bar", "name": "exmple2"}
            ]
        }
    ),

    html.H2("*** Line Graph *** "),
    dcc.Graph(
        id="exm2",
        figure={
            "data": [
                {"x": [1, 3, 5], "y": [10, 12, 14], "type": "line", "name": "exmple3"},
                {"x": [2, 4, 6], "y": [13, 15, 17], "type": "line", "name": "exmple4"}
            ]
        }
    ),

    html.H2("*** Scatter Plot Graph *** "),
    dcc.Graph(
        id="exm3",
        figure={
            "data": [
                {"x": [1, 3, 5, 7], "y": [10, 12, 14, 16], "type": "scatter", "mode": "markers", "name": "scatter exmpl1"},
                {"x": [2, 4, 6, 8], "y": [13, 15, 17, 19], "type": "scatter", "mode": "markers", "name": "scatter exmpl2"}
            ]
        }
    ),

    html.H2("*** Pie Chart Graph *** "),
    dcc.Graph(
        id="exm4",
        figure={
            "data": [
                {"labels": ["A", "B", "C"], "y": [10, 12, 14], "type": "pie", "name": "pie chart expl1"},
            ],
            "layout": {"title": "pie chart example"}
        }
    )
])
